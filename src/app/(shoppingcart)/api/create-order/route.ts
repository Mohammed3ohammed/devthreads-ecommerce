...
const manageOrderInDB = async (paymentIntent: any, total: number, items:CartEntry[], userId: string) => {
   ...
   if (existingOrder) {
    return await prisma?.order.update (
        {
            where:{paymentIntentID: paymentIntent.id},
           data: {userId, amount:total, currency: "usd",
...
const {items, payment_initent_id, totalAmount} = body
const total = totalAmount

try {
    const paymentIntent = await manageStripePaymentIntent(payment_initent_id, total)
